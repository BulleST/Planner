...
export class FormCarteiraSetupComponent implements OnInit, OnChanges {
    ...
    _carteiraSetup: CarteiraSetup = undefined as unknown as CarteiraSetup;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes['objeto']){
            ...
            if (this.objeto.carteiraSetup_Id != 0) {
                ...
                this._carteiraSetup = undefined as unknown as CarteiraSetup;
            }
        }
    ...
    carteiraSetupChange() {
        this.novaCarteiraSetup = this._carteiraSetup == undefined;
        if (this.novaCarteiraSetup) {
            this.novaCarteira();
        } else {
            this.objeto.carteiraSetup = this._carteiraSetup.nome;
            this.objeto.carteiraSetup_Id = this._carteiraSetup.id;
            this._carteiraSetup = this.empresaService.object.carteiraSetup.find(x => x.id == this.objeto.carteiraSetup_Id) as unknown as CarteiraSetup;
            this.objeto.produtoTributacaoRel = this._carteiraSetup.carteiraProdutoRel.map(x => x.produtoTributacaoRel);
        }
    }
    ...
    removeItem(id: number) {
        ...
        if (index != -1) {
            this.objeto.produtoTributacaoRel.splice(index, 1);
        }
    }
}
